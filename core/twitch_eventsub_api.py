...
class TwitchEventSubClient:
    ...
    async def _subscribe_all(self):
        events = [
            ...
            ('channel.hype_train.begin', '2', {'broadcaster_user_id': self.user_id}),
            ('channel.hype_train.progress', '2', {'broadcaster_user_id': self.user_id}),
            ('channel.hype_train.end', '2', {'broadcaster_user_id': self.user_id}),
            ('channel.goal.begin', '1', {'broadcaster_user_id': self.user_id}),
            ('channel.goal.progress', '1', {'broadcaster_user_id': self.user_id}),
            ('channel.goal.end', '1', {'broadcaster_user_id': self.user_id}),
        ]
    ...
    async def _process_event(self, event_type, event):
        try:
            if event_type == 'channel.follow':
                ...
                total = event.get('total', 0)
                user = event.get('user_name', 'Сообщество')
                data = {
                    "type": "hype_train",
                    "user": user,
                    "level": level,
                ...
                logger.info(f"🔥 Хайповоз: {user} (уровень {level}, всего {total})")
                self.callback("event", data)
            elif event_type in ('channel.goal.begin', 'channel.goal.progress', 'channel.goal.end'):
                goal_type = event.get('type', '')
                goal_name = event.get('description', '')
                current_amount = event.get('current_amount', 0)
                target_amount = event.get('target_amount', 0)
                data = {
                    "type": "goal",
                    "user": "Система",
                    "goal_type": goal_type,
                ...
                logger.info(f"🎯 Цель: {goal_name} ({current_amount}/{target_amount})")
                self.callback("event", data)
            else:
